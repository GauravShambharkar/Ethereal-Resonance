...
const SoundSelectionModal = ({
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-md px-4">
      <div className="glass-modal w-full max-w-2xl p-8 rounded-2xl flex flex-col gap-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {coreSounds.map((sound) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-3 p-4 rounded-xl border transition-all duration-500 group ${
                    oscillatorType === sound.id
                ...
                >
                  <span className={`material-symbols-outlined text-2xl transition-all duration-500 ${
                    oscillatorType === sound.id ? "text-primary scale-110" : "text-on-surface-variant group-hover:text-primary"
                  }`}>
                    {sound.icon}
                  </span>
                  <span className={`font-headline text-[10px] tracking-widest uppercase transition-colors ${
                    oscillatorType === sound.id ? "text-primary" : "text-on-surface group-hover:text-tertiary"
                  }`}>
                    {sound.label}
                  </span>
                </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {etherealSounds.map((sound) => (
                <button
                  ...
                  className={`flex items-center gap-4 p-4 rounded-xl border transition-all duration-500 group ${
                    oscillatorType === sound.id
                      ? "border-primary bg-primary/10 shadow-[0_0_20px_rgba(170,202,234,0.15)]"
                      : "border-outline-variant/20 bg-surface-container-low hover:border-primary/40 hover:bg-surface-bright"
                  }`}
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-500 ${
                    oscillatorType === sound.id ? "bg-primary text-on-primary" : "bg-surface-container-highest text-on-surface-variant group-hover:bg-primary/20 group-hover:text-primary"
                  }`}>
                  ...
                  <div className="text-left">
                    <span className={`font-headline text-[10px] block tracking-widest uppercase mb-0.5 transition-colors ${
                      oscillatorType === sound.id ? "text-primary" : "text-on-surface group-hover:text-tertiary"
                    }`}>
                      {sound.label}
                    </span>
